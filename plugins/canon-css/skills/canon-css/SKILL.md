...
COMPONENTS - `data-component` on semantic elements:
    ...
    topbar    > data-slot={brand|nav|actions|menu}. menu = mobile burger,
              ...
              replaces the inline nav on mobile. The burger is drawn by
              Canon; the word you write is hidden and stays as the
              accessible name, so never put an icon character there.
